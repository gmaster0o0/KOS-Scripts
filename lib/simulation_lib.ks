...
global simulationLib to ({
  local function simulateThrust {
    parameter startTime,
              ...
              thrustVector, 
              endTime. 
    parameter shipThrust is ship:availablethrust. 
    parameter engineIsp is RocketUtils:getAvarageISP().
    parameter currentBody is body.
    parameter minDt is 0.02. 
    parameter maxDt is 0.06.
  ...
  local function simulate {
    parameter startState, shipInfo, settings.

    local minDT to settings:minDt.
    local maxDT to choose settings:maxDt if settings:hasKey("maxDt") else minDT * 5.
    local dt to minDT.
    local hdt to dt / 2.
    local simTime is startState:startTime.
    ...
    local massFlowrate is shipInfo:shipThrust / shipInfo:engineIsp.
    local massFlowrateDt is massFlowrate * dt.
    
    local adaptiveDt to true.
    local stepError to 0.

    local calculateAcceleration to settings:calculateAcceleration.
    local loopCondition to settings:loopCondition.
    local refineCondition to settings:refineCondition.
    ...
    until false {
      until loopCondition(simState, simPreviousState){
        ...
        set simMass to simMass - massFlowrateDt.
        ...
        if adaptiveDt {
            ...
            if (stepError < 0.1 and dt < maxDt) or (stepError > 1 and dt > minDt) {
                ...
                set hdt to dt / 2.
                set massFlowrateDt to massFlowRate * dt.
            }
        }
      }
  ...
  local function getCalcAccFixedDirThrustFunction {
      ...
      return {
          ...
          //acceleration vector = ship:acc - gravity vector
          return (-pos:normalized) * (bodyMu / pos:sqrmagnitude) +    // gravity
                  thrustVector * (shipThrust / massParam).            // thrust a = F/m
      }.
  }
  ...
  local function getCalcAccRetroThrustRotFunction {
    ...
    return {
      ...
      return (-pos:normalized) * (bodyMu / pos:sqrmagnitude) +     // gravity F= G * (m1*m2)/r^2=(mu*m)/r˘2 mu=GM => a=g=mu/r^2
              (-2 * vcrs(bodyAngularVel, vel)) +                   // coriolis: F= -2m*w x v F=m*a => a = -2w x v
              vcrs(bodyAngularVel, vcrs(bodyAngularVel, -pos)) +   // centrifugal F= m*w^2 * r
              (-vel:normalized) * (shipThrust / massParam).        // thrust  a = F/m
    }.
  }
