...
global hillClimbLib is ({
  ...
  // Convenience wrapper for searching tree dimensions.
  local function ddd_search {
    parameter cost, x, y, z, scale_y, scale_z, minimum, step_size, step_threshold.

    local dimensions is list(v(1, 0, 0), v(-1, 0, 0), v(0, scale_y, 0), v(0, -scale_y, 0), v(0, 0, scale_z), v(0, 0, -scale_z)).
    local position is v(x, y, z).
